...
!function () {
  ...
  buildDisCorr = function (data) {
    ...
    linesPath.enter().append('path')
      .attr('d', function (d, i, j) {
        var k = keys[j];
        line.y(function (d) {
          return axes.lineY(d[k]);
        })
        return line(d, i);
      })
      .attr('stroke', function (d, i, j) {
        return getColor(keys[j])
      })
      .attr('stroke-width', 2)
      .attr('fill', 'none');
  ...
  buildTextBox = function (data) {
    ...
    var textScale = d3.domain(o.map(function (d) {
      return d.key
    }))
      .scaleOrdinal(d3.range(0, 81, 27));

    var ttLabel = tWrap.selectAll('text.tt-label').data(o);
    ttLabel.enter().append('text')
      .attr('class', function (d) {
        return 'tt-label ' + d.key
      })
      .attr('x', function (d, i) {
        return i < 3 ? 18 : scatterWidth / 2 + 20
      })
      // .attr('text-anchor', 'end')
      .attr('y', function (d, i) {
        return textScale(d.key) + i % 3 * 27
      })
      .attr('fill', getColor)
      .text(function (d) {
        return d.label
    ...
    ttVal.enter().append('text')
      .attr('class', function (d) {
        return 'tt-value ' + d.key
      })
      .attr('y', function (d, i) {
        return textScale(d.key) + (i % 3 * 27) + 18
      })
      .attr('x', function (d, i) {
        return i < 3 ? 18 : scatterWidth / 2 + 20
      })
        ...
        if (point.date === k.date) {
          d3.select(this)
            .attr('fill', function (d) {
              return d.avgAlpha < 0 ? '#DAA0A2' : '#C8DDEC'
            })
  ...
  buildScatter = function (data) {
    ...
    circles.enter().append('circle')
      ...
      .attr('cx', function (d) {
        return axes.scatterX(d.corr)
      })
      .attr('cy', function (d) {
        return axes.scatterY(d.disp)
      })
    ...
    var vPath = vWrap.selectAll('path').data(function (d) {
      return voronoi(d).filter(function (d) {
        return d;
      });
    });
  ...
  Axes = function (data) {
    ...
    var minD = d3.min(data, function (d) {
      return d.disp;
    });
    ...
    var maxFOF = d3.max(data, function (d) {
      return d.fof;
    });
    var maxHFRI = d3.max(data, function (d) {
      return d.hfri;
    });
    var maxHFU = d3.max(data, function (d) {
      return d.hfu;
    });


    var minFOF = d3.min(data, function (d) {
      return d.fof;
    });
    var minHFRI = d3.min(data, function (d) {
      return d.hfri;
    });
    var minHFU = d3.min(data, function (d) {
      return d.hfu;
    });
    ...
    var medD = d3.median(data, function (d) {
      return d.disp;
    });
    var medC = d3.median(data, function (d) {
      return d.corr;
    });
